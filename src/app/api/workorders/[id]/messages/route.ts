...
export const GET = withApiError(
  async (req: Request, context: { params: Promise<{ id: string }> }) => {
    ...
    const reference = req.headers.get("x-repair-reference");
    if (!user && reference !== order.orderNumber)
      return Response.json(
        { error: "Repair reference required" },
        { status: 403 },
      );
  ...
  async (req: Request, context: { params: Promise<{ id: string }> }) => {
    ...
    if (senderType === "CUSTOMER" && order.shopId) {
      try {
        ...
        await Promise.all(
          adminIds.map(async (uid) => {
            await createNotification(
              uid,
              "NEW_MESSAGE",
              `Customer message on order ${shortId}`,
              {
                workOrderId: params.id,
        ...
        console.error("Message saved; notification delivery failed");
      }
    }
