      ...
      <div className="max-w-3xl mx-auto bg-white rounded-xl shadow-2xl p-8 text-gray-900 print:shadow-none print:rounded-none print:p-6">
        ...
        <div className="flex items-start justify-between mb-6 pb-6 border-b border-gray-200">
          ...
          <div className="text-right">
            <h1 className="text-2xl font-bold text-gray-900">Device Health Report</h1>
            <p className="text-xs text-gray-500 mt-1">Post-Repair Condition Assessment</p>
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          <div className="bg-gray-50 rounded-lg p-4">
            ...
            <div className="space-y-1.5">
              <div><p className="text-xs text-gray-400">Name</p><p className="text-sm font-medium">{order.customerName}</p></div>
              <div><p className="text-xs text-gray-400">Phone</p><p className="text-sm font-medium">{order.customerPhone}</p></div>
              {order.customerEmail && <div><p className="text-xs text-gray-400">Email</p><p className="text-sm font-medium">{order.customerEmail}</p></div>}
            </div>
          </div>
          <div className="bg-gray-50 rounded-lg p-4">
            <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wide mb-3">Device</h3>
            <div className="space-y-1.5">
              <div><p className="text-xs text-gray-400">Device</p><p className="text-sm font-medium">{order.deviceBrand} {order.deviceModel}</p></div>
              {order.serialNumber && <div><p className="text-xs text-gray-400">Serial Number</p><p className="text-sm font-mono">{order.serialNumber}</p></div>}
              {order.imei && <div><p className="text-xs text-gray-400">IMEI</p><p className="text-sm font-mono">{order.imei}</p></div>}
            </div>
          </div>
        </div>
        ...
        {/* Repair Summary */}
        <div className="mb-6">
          <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wide mb-3">Repair Summary</h3>
          <div className="bg-gray-50 rounded-lg p-4 space-y-2">
            <div className="flex justify-between text-sm"><span className="text-gray-500">Issue Reported</span><span className="font-medium text-right max-w-xs">{order.faultDescription}</span></div>
            {order.repairType && <div className="flex justify-between text-sm"><span className="text-gray-500">Repair Type</span><span className="font-medium">{order.repairType}</span></div>}
            <div className="flex justify-between text-sm"><span className="text-gray-500">Received</span><span className="font-medium">{new Date(order.receivedAt).toLocaleDateString()}</span></div>
            {order.doneAt && <div className="flex justify-between text-sm"><span className="text-gray-500">Completed</span><span className="font-medium">{new Date(order.doneAt).toLocaleDateString()}</span></div>}
            <div className="flex justify-between text-sm"><span className="text-gray-500">Technician</span><span className="font-medium">{order.assignee?.name ?? order.creator.name}</span></div>
          </div>
        </div>
        ...
        {order.parts.length > 0 && (
          <div className="mb-6">
            <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wide mb-3">Parts Replaced</h3>
            <table className="w-full text-sm">
              <thead><tr className="border-b border-gray-200">
                <th className="text-left py-2 text-xs text-gray-400 font-medium">Part</th>
                <th className="text-left py-2 text-xs text-gray-400 font-medium">Part #</th>
                <th className="text-right py-2 text-xs text-gray-400 font-medium">Qty</th>
                <th className="text-right py-2 text-xs text-gray-400 font-medium">Total</th>
              </tr></thead>
              <tbody>
                {order.parts.map(p => (
        ...
        {order.checklist.length > 0 && (
          <div className="mb-6">
            <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wide mb-3">Device Condition Assessment</h3>
            <div className="grid grid-cols-3 gap-2 mb-3">
              ...
              <div className="bg-red-50 rounded-lg p-2 text-center"><p className="text-lg font-bold text-red-600">{checkIssue.length}</p><p className="text-xs text-red-600">Issues</p></div>
        ...
        <div className="text-center text-xs text-gray-400 border-t border-gray-200 pt-4">
          <p>This health report certifies the device was repaired and tested by {order.shop?.name ?? "FixFlow"}.</p>
          <p className="mt-1">Generated by FixFlow · {new Date().toLocaleString()}</p>
        </div>
      </div>
    </div>
  );
