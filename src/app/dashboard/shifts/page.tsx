  ...
  return (
    <div className="p-6 space-y-6 max-w-5xl mx-auto">
        ...
        {activeShift ? (
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              ...
              <span className="font-semibold text-slate-100">Active Shift</span>
            </div>
            <p className="text-sm text-slate-400">Started at {fmt(activeShift.startTime)} — {duration(activeShift.startTime, null)}</p>
          ...
          <div className="space-y-3">
            <p className="text-sm font-medium text-slate-400">No active shift</p>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-800"><h2 className="font-semibold text-slate-300 text-sm">History</h2></div>
              ...
              <tbody className="divide-y divide-slate-800">
                {shifts.map((s) => (
                  <tr key={s.id} className="hover:bg-slate-800/50 transition">
                    <td className="px-6 py-3 font-medium text-slate-200">{s.user.name}</td>
                    <td className="px-6 py-3 text-slate-400">{fmt(s.startTime)}</td>
                    <td className="px-6 py-3">{s.endTime ? <span className="text-slate-400">{fmt(s.endTime)}</span> : <span className="text-green-400 font-medium text-xs">Active</span>}</td>
                    <td className="px-6 py-3 text-slate-400">{duration(s.startTime, s.endTime)}</td>
                    <td className="px-6 py-3 text-slate-400 max-w-[160px] truncate">{s.notes ?? "—"}</td>
                  </tr>
                ))}
              </tbody>
            </table>
