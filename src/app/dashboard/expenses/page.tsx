...
const CATEGORY_COLORS: Record<string, string> = {
  RENT: "bg-blue-500/20 text-blue-600 dark:text-blue-400",
  UTILITIES: "bg-yellow-500/20 text-yellow-600 dark:text-yellow-400",
  PARTS_PURCHASE: "bg-orange-500/20 text-orange-600 dark:text-orange-400",
  SALARY: "bg-purple-500/20 text-purple-600 dark:text-purple-400",
  MARKETING: "bg-pink-500/20 text-pink-600 dark:text-pink-400",
  EQUIPMENT: "bg-cyan-500/20 text-cyan-600 dark:text-cyan-400",
    ...
    <div className="p-6 space-y-6 max-w-5xl mx-auto">
      ...
      <div className="flex gap-2 flex-wrap">
        <button onClick={() => setCategoryFilter("")}
          className={`px-3 py-1.5 text-xs rounded-lg border transition-colors ${!categoryFilter ? "bg-blue-600 text-white border-blue-600" : "bg-white dark:bg-slate-900 text-slate-500 dark:text-slate-400 border-slate-200 dark:border-slate-800 hover:border-slate-400 dark:hover:border-slate-600"}`}>
          All
        </button>
        {CATEGORIES.map(c => (
          <button key={c} onClick={() => setCategoryFilter(c)}
            className={`px-3 py-1.5 text-xs rounded-lg border transition-colors ${categoryFilter === c ? "bg-blue-600 text-white border-blue-600" : "bg-white dark:bg-slate-900 text-slate-500 dark:text-slate-400 border-slate-200 dark:border-slate-800 hover:border-slate-400 dark:hover:border-slate-600"}`}>
            {c.replace("_", " ")}
          </button>
        ))}
      </div>
          ...
          <tbody>
            {loading && [...Array(6)].map((_, i) => (
              <tr key={i} className="border-b border-slate-200/50 dark:border-slate-800/50 animate-pulse">
                ...
                <td className="px-4 py-3.5"><div className={`h-3 bg-slate-200 dark:bg-slate-700 rounded ${["w-32","w-28","w-36","w-24","w-32","w-28"][i]}`} /></td>
                <td className="px-4 py-3.5"><div className="h-5 w-20 bg-slate-200 dark:bg-slate-800 rounded-full" /></td>
                <td className="px-4 py-3.5"><div className={`h-3 bg-slate-200 dark:bg-slate-800 rounded ${["w-16","w-20","w-14","w-18","w-16","w-20"][i]}`} /></td>
                <td className="px-4 py-3.5"><div className={`h-3 bg-slate-200 dark:bg-slate-800 rounded ${["w-24","w-16","w-28","w-20","w-0","w-24"][i]}`} /></td>
                <td className="px-4 py-3.5"><div className="h-3 w-16 bg-slate-200 dark:bg-slate-800 rounded" /></td>
