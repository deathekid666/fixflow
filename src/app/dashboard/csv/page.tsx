...
export default function CsvPage() {
  ...
  return (
    <div className="p-6 space-y-6 max-w-4xl mx-auto">
      <div><h1 className="text-2xl font-bold text-slate-100">CSV Import</h1><p className="text-sm text-slate-400 mt-1">Bulk import spare parts or customers</p></div>
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 space-y-4">
        <p className="text-sm font-semibold text-slate-300">1. Import type</p>
        <div className="flex gap-3 flex-wrap">
          {(["spareparts", "customers"] as ImportType[]).map((t) => (
            <button key={t} onClick={() => { setType(t); reset(); }}
              className={`px-5 py-2 rounded-lg border text-sm font-medium transition ${type === t ? "bg-blue-600 text-white border-blue-600" : "bg-slate-800 text-slate-300 border-slate-700 hover:border-blue-400"}`}>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 space-y-4">
        <p className="text-sm font-semibold text-slate-300">2. Upload CSV</p>
      ...
      {result && (
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 space-y-4">
          <p className="text-sm font-semibold text-slate-300">3. Results</p>
          <div className="flex flex-wrap gap-3">
            <div className="px-4 py-2 bg-slate-800 rounded-lg text-sm text-slate-300">Total: <strong>{result.totalRows}</strong></div>
            <div className="px-4 py-2 bg-green-900/40 rounded-lg text-sm text-green-400">✓ Imported: <strong>{result.imported}</strong></div>
            <div className="px-4 py-2 bg-red-900/40 rounded-lg text-sm text-red-400">✗ Skipped: <strong>{result.skipped}</strong></div>
          </div>
          {result.errors.length === 0 ? <p className="text-sm text-green-400 font-medium">✅ All rows imported successfully!</p> : (
            <div>
              <p className="text-sm font-semibold text-red-400 mb-2">Errors ({result.errors.length} rows)</p>
              <div className="overflow-x-auto rounded-lg border border-slate-700">
                <table className="w-full text-xs">
                  <thead className="bg-slate-800 text-slate-400"><tr><th className="px-4 py-2 text-left">Row</th><th className="px-4 py-2 text-left">Reason</th><th className="px-4 py-2 text-left">Data</th></tr></thead>
                  <tbody className="divide-y divide-slate-800">
                    {result.errors.map((e, i) => (
                      <tr key={i} className="bg-slate-900">
                        <td className="px-4 py-2 text-slate-400">#{e.row}</td>
                        <td className="px-4 py-2 text-red-400">{e.reason}</td>
