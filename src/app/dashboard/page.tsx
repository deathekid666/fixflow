    ...
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => load()} className="px-3 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 text-sm rounded-lg transition-colors">
            🔄 Refresh
          </button>
          <Link href="/dashboard/workorders/new" className="px-4 py-2 bg-blue-600 hover:bg-blue-500 text-white text-sm rounded-lg transition-colors font-medium">
            + New Work Order
          </Link>
        </div>
      </div>
        ...
        <div className="flex gap-2 flex-wrap">
          {["", "RECEIVED", "DIAGNOSING", "REPAIRING", "DONE", "DELIVERED", "CANCELLED"].map((s) => (
            <button key={s} onClick={() => setStatusFilter(s)}
              className={`px-3 py-2 text-xs rounded-lg border font-medium transition-colors ${statusFilter === s ? "bg-blue-600 text-white border-blue-600" : "bg-slate-900 text-slate-400 border-slate-800 hover:border-slate-600"}`}>
              {s || "All"}
            </button>
          ))}
        </div>
      ...
      {selected.size > 0 && (
        <div className="bg-blue-950/40 border border-blue-800/50 rounded-xl px-4 py-3 flex items-center gap-4 flex-wrap">
          ...
          <button onClick={exportSelected} className="px-3 py-1.5 bg-slate-700 hover:bg-slate-600 text-slate-300 text-xs rounded-lg transition-colors ml-auto">
