...
function Stars({ n }: { n: number }) {
  return <span><span className="text-yellow-400">{"★".repeat(n)}</span><span className="text-slate-600">{"★".repeat(5 - n)}</span></span>;
}
...
export default function RatingsPage() {
  ...
  return (
    <div className="p-6 space-y-6 max-w-4xl mx-auto">
      <div>
        <h1 className="text-2xl font-bold text-slate-100">Customer Satisfaction</h1>
        <p className="text-sm text-slate-400 mt-1">Ratings collected after delivery</p>
      </div>
      {!loading && ratings.length > 0 && (
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-6 flex flex-col sm:flex-row gap-8 items-start">
          <div className="text-center min-w-[100px]">
            <div className="text-5xl font-bold text-slate-100">{avg}</div>
            <div className="text-yellow-400 text-xl mt-1">{"★".repeat(Math.round(parseFloat(avg!)))}</div>
      ...
      {loading ? <p className="text-sm text-slate-400">Loading...</p> : error ? <p className="text-sm text-red-400">{error}</p> : ratings.length === 0 ? (
        ...
        <div className="space-y-3">
          {ratings.map((r) => (
            <div key={r.id} className="bg-slate-900 border border-slate-800 rounded-xl p-4 space-y-1">
              <div className="flex items-center justify-between flex-wrap gap-2">
                <div>
                  <p className="font-medium text-slate-200 text-sm">{r.workOrder.customerName} <span className="text-slate-400 font-normal text-xs">{r.workOrder.customerPhone}</span></p>
