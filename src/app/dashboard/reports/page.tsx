...
const TYPE_COLORS: Record<string, string> = {
  ADD: "text-green-400", REMOVE: "text-red-400", CORRECTION: "text-yellow-400",
};
      ...
      <div className="p-4 md:p-6 space-y-6 max-w-5xl mx-auto">
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          ...
          <button
            ...
            className="no-print flex items-center gap-2 px-4 py-2 bg-slate-800 hover:bg-slate-700 text-slate-300 text-sm rounded-lg transition-colors"
          >
          ...
          <>
              ...
              <div className="flex gap-2 flex-wrap no-print">
                {years.map(y => (
                  <button key={y} onClick={() => setYear(y)}
                    className={`px-4 py-1.5 rounded-lg text-sm font-medium transition-colors ${year === y ? "bg-blue-600 text-white" : "bg-slate-800 text-slate-400 hover:text-white"}`}>
                    {y}
                  </button>
                ))}
              </div>
            ...
            <div className="print-section">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
                {[
                  { label: "Revenue", value: `${fmt(ys.revenue)} MAD`, color: "text-white", sub: `${ys.orders} orders` },
                  { label: "Collected", value: `${fmt(ys.collected)} MAD`, color: "text-green-400", sub: `${collectionRate}% rate` },
                  { label: "Outstanding", value: `${fmt(ys.revenue - ys.collected)} MAD`, color: ys.revenue - ys.collected > 0 ? "text-yellow-400" : "text-slate-500", sub: "uncollected" },
                  { label: "Expenses", value: `${fmt(ys.expenses)} MAD`, color: "text-red-400", sub: "total costs" },
                  { label: "Net Profit", value: `${fmt(ys.profit)} MAD`, color: ys.profit >= 0 ? "text-emerald-400" : "text-red-400", sub: ys.collected > 0 ? `${Math.round((ys.profit / ys.collected) * 100)}% margin` : "" },
                  { label: "Orders", value: ys.orders.toString(), color: "text-blue-400", sub: `avg ${ys.orders > 0 ? fmt(ys.revenue / ys.orders) : "0"} MAD` },
                ].map(s => (
                  <div key={s.label} className="print-card bg-slate-900 border border-slate-800 rounded-xl p-4">
            ...
            <div className="print-section bg-slate-900 border border-slate-800 rounded-xl overflow-hidden print-card">
              <div className="px-5 py-4 border-b border-slate-800">
                <h2 className="text-sm font-semibold text-slate-200">Month-by-Month Breakdown — {year}</h2>
              </div>
              {filtered.length === 0 ? (
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {filtered.map((m, i) => {
                        ...
                        return (
                          <tr key={m.label} className={`print-row-alt border-b border-slate-800/50 hover:bg-slate-800/30 transition-colors ${!isLastRow ? "" : ""}`}>
                            <td className="px-4 py-3 text-white font-medium">{monthLabel(m.label)}</td>
                            <td className="px-4 py-3 text-slate-300">{m.count}</td>
                            <td className="px-4 py-3 text-white">{fmt(m.total)}</td>
                            <td className="px-4 py-3 text-green-400">{fmt(m.collected)}</td>
                            <td className="px-4 py-3">
                              <div className="flex items-center gap-2">
                                ...
                                <span className={`text-xs font-medium ${rate >= 80 ? "text-green-400" : rate >= 50 ? "text-yellow-400" : "text-red-400"}`}>{rate}%</span>
                              </div>
                            </td>
                            <td className="px-4 py-3 text-red-400">{fmt(m.expenses)}</td>
                            <td className={`px-4 py-3 font-semibold ${m.profit >= 0 ? "text-emerald-400" : "text-red-400"}`}>{fmt(m.profit)}</td>
                          </tr>
                        );
                      })}
                      {/* Totals row */}
                      <tr className="bg-slate-800/40 border-t-2 border-slate-700">
                        <td className="px-4 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wider">Total</td>
                        <td className="px-4 py-3 text-white font-semibold">{ys.orders}</td>
                        <td className="px-4 py-3 text-white font-semibold">{fmt(ys.revenue)}</td>
                        <td className="px-4 py-3 text-green-400 font-semibold">{fmt(ys.collected)}</td>
                        <td className="px-4 py-3 text-xs font-semibold">{collectionRate}%</td>
                        <td className="px-4 py-3 text-red-400 font-semibold">{fmt(ys.expenses)}</td>
                        <td className={`px-4 py-3 font-bold ${ys.profit >= 0 ? "text-emerald-400" : "text-red-400"}`}>{fmt(ys.profit)}</td>
                      </tr>
                    </tbody>
                  </table>
                </div>
            ...
            <div className="no-print">
              <div className="pt-2 border-t border-slate-800">
                <h2 className="text-base font-semibold text-white mb-4">Parts Report</h2>
              </div>
                ...
                <div className="space-y-5">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
                      ...
                      <p className="text-2xl font-semibold text-white mt-1">{parts.totals.totalQuantity}</p>
                    </div>
                    <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
                      <p className="text-xs text-slate-500">Parts Revenue</p>
                      <p className="text-2xl font-semibold text-green-400 mt-1">{fmt(parts.totals.totalRevenue)}</p>
                    </div>
                  </div>

                  <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
                    <h3 className="text-sm font-semibold text-slate-300 mb-4">Parts Consumption</h3>
                          ...
                          <tbody>
                            {parts.usage.map(u => (
                              <tr key={u.sparePartId} className="border-b border-slate-800/50">
                                <td className="py-2 px-1 text-white font-medium">{u.part?.name ?? "—"}</td>
                                <td className="py-2 px-1 font-mono text-xs text-slate-400">{u.part?.partNumber ?? "—"}</td>
                                <td className="py-2 px-1 text-slate-300">{u._count.id}</td>
                                <td className="py-2 px-1 text-slate-300">{u._sum.quantity}</td>
                                <td className="py-2 px-1 text-white font-medium">{fmt(u._sum.total ?? 0)}</td>
                  ...
                  <div className="bg-slate-900 border border-slate-800 rounded-xl p-5">
                    <h3 className="text-sm font-semibold text-slate-300 mb-4">Stock Adjustment History</h3>
                          ...
                          <tbody>
                            {parts.adjustments.map(a => (
                              <tr key={a.id} className="border-b border-slate-800/50">
                                <td className="py-2 px-1 text-white">{a.sparePart.name}</td>
                                <td className="py-2 px-1"><span className={`font-medium text-xs ${TYPE_COLORS[a.type]}`}>{a.type}</span></td>
                                <td className="py-2 px-1 text-slate-300">{a.quantity}</td>
