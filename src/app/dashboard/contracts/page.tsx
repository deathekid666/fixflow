  ...
  return (
    <div className="p-4 sm:p-6 space-y-5 max-w-5xl mx-auto">
      ...
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <div>
          <h1 className="text-xl font-semibold text-slate-900 dark:text-white">Maintenance Contracts</h1>
          <p className="text-sm text-slate-500 mt-0.5">Recurring service agreements with automatic work order generation</p>
        </div>
        <button
      ...
      {billedCount !== null && billedCount > 0 && (
        <div className="bg-green-500/10 border border-green-500/30 rounded-xl px-4 py-3 flex items-center gap-3">
          ...
          <button onClick={() => setBilledCount(null)} className="ml-auto text-green-600 dark:text-green-400 text-xs hover:underline">Dismiss</button>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: "Active Contracts", value: activeContracts.length, icon: "📄", color: "text-blue-600 dark:text-blue-400" },
          { label: "Monthly Revenue", value: fmt(monthlyRevenue), icon: "💰", color: "text-green-600 dark:text-green-400" },
          { label: "Annual Revenue", value: fmt(monthlyRevenue * 12), icon: "📈", color: "text-emerald-600 dark:text-emerald-400" },
          { label: "Due for Billing", value: dueCount, icon: "⏰", color: dueCount > 0 ? "text-orange-500" : "text-slate-500" },
        ].map(s => (
      ...
      {showForm && (
        <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-5 space-y-4">
          <h2 className="text-sm font-semibold text-slate-600 dark:text-slate-300">New Contract</h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div className="sm:col-span-2">
              <label className="text-xs text-slate-400 mb-1 block">Service Description</label>
            ...
            <div>
              <label className="text-xs text-slate-400 mb-1 block">Monthly Price ({currency})</label>
            ...
            <div>
              ...
              <input value={form.notes} onChange={e => setForm(p => ({ ...p, notes: e.target.value }))} placeholder="Internal notes" className={INPUT} />
            </div>
          </div>
          <button
      ...
      {/* Contracts list */}
      {loading ? (
        <div className="text-center py-16 text-slate-400 text-sm">Loading contracts…</div>
      ) : filtered.length === 0 ? (
        <div className="text-center py-16 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl">
          <p className="text-3xl mb-3">📄</p>
          <p className="text-slate-500 text-sm font-medium">No {filter !== "ALL" ? filter.toLowerCase() + " " : ""}contracts yet</p>
          <p className="text-slate-400 text-xs mt-1">Create a contract to start tracking recurring maintenance revenue</p>
        </div>
      ) : (
              ...
              <div key={c.id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden">
                ...
                <div className="p-4 flex flex-wrap items-start gap-3">
                  ...
                  <div className="text-right flex-shrink-0">
                    ...
                    <p className={`text-xs mt-0.5 ${billing.cls}`}>Next: {billing.label}</p>
                    {c.endDate && (
                      <p className="text-xs text-slate-400 mt-0.5">Ends {new Date(c.endDate).toLocaleDateString()}</p>
                    )}
                  </div>
                ...
                {isEditing && (
                  <div className="border-t border-slate-100 dark:border-slate-800 p-4 bg-slate-50 dark:bg-slate-800/50 space-y-3">
                    <p className="text-xs font-semibold text-slate-400 uppercase tracking-wide">Edit Contract</p>
