...
export function SkeletonTable({ rows = 5 }: { rows?: number }) {
  return (
    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl overflow-hidden">
      <div className="border-b border-slate-200 dark:border-slate-800 px-4 py-3 flex gap-4 animate-pulse">
        {[1,2,3,4,5].map(i => <div key={i} className="h-3 bg-slate-200 dark:bg-slate-700 rounded w-20" />)}
      </div>
